...
export function PrincipalContainer(){
    ...
    return(
        <>
            ...
            <PersonCardName name={user?.firstName} email={user?.email} orders={user?.totalOrders} memberSince={`${user?.memberSince ? new Date(user.memberSince).toISOString().split('-')[0] : "Unknown"}`}/>
            <ProfileContainer/>
